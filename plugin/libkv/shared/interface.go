// shared contains shared data between the host and plugins
package shared
...
// Handshake is a common handshake shared by plugin and host
...
// KV is the interface that is exposed as a plugin
type KV interface {
	Put(key string, value []byte) error
	Get(key string) ([]byte, error)
}

// Implementation of plugin.Plugin to serve/consumer. GRPCPlugin is also implemented so it can be served over gRPC.
type KVPlugin struct {
	// Concreate implementation in Go
	Impl KV
}
